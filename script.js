...
const adminConfigStorageKey = "timenest-admin-config-v1";
...
const defaultAdminConfig = {
  ...
  yearlyPrice: 1499,
  currency: "INR",
  offerHeadline: "Founding member pricing",
  offerMessage:
    "Keep TimeNest free while you grow. Turn on premium plans from the admin panel when retention is strong enough.",
  supportEmail: "founder@timenest.app",
  premiumFeatures: {
    cloudBackup: true,
...
const featureDisplayCopy = {
  cloudBackup: {
    included: "Cloud backup stays included for every account and keeps cross-device restore simple.",
    planned:
      "Cloud backup is prepared as a premium feature that can go live whenever you are ready.",
    live: "Cloud backup and multi-device restore are now part of the premium plan."
  },
  advancedAnalytics: {
    included: "Advanced analytics remain available to all users.",
    planned:
      "Deep productivity scorecards can stay free for now and move into premium later.",
    live: "Advanced analytics and scorecards are now reserved for premium members."
  },
  advancedExports: {
    included: "PDF and Excel exports stay available for every user.",
    planned:
      "Exports are ready to become a premium power-user feature when paid plans launch.",
    live: "PDF and Excel exports are now locked to premium members."
  },
  whatsappReminders: {
    included: "WhatsApp reminders stay available for every user.",
    planned:
      "WhatsApp reminders are set up as a premium-only reminder channel for the future launch.",
    live: "WhatsApp escalation reminders are now a premium-only channel."
  }
};
...
function normalizeAdminConfig(rawConfig = {}) {
  const defaults = cloneDefaultAdminConfig();
  const rawFeatures =
    rawConfig && typeof rawConfig.premiumFeatures === "object" ? rawConfig.premiumFeatures : {};

  return {
    launchMode: rawConfig.launchMode === "live" ? "live" : "grow",
    freeAccessMonths: sanitizePositiveInteger(rawConfig.freeAccessMonths, defaults.freeAccessMonths),
    trialDays: sanitizePositiveInteger(rawConfig.trialDays, defaults.trialDays),
    monthlyPrice: sanitizePositiveInteger(rawConfig.monthlyPrice, defaults.monthlyPrice),
    yearlyPrice: sanitizePositiveInteger(rawConfig.yearlyPrice, defaults.yearlyPrice),
    currency: sanitizeText(rawConfig.currency, defaults.currency).slice(0, 3).toUpperCase(),
    offerHeadline: sanitizeText(rawConfig.offerHeadline, defaults.offerHeadline),
    offerMessage: sanitizeText(rawConfig.offerMessage, defaults.offerMessage),
    supportEmail: sanitizeText(rawConfig.supportEmail, defaults.supportEmail),
    premiumFeatures: {
      cloudBackup:
        typeof rawFeatures.cloudBackup === "boolean"
          ? rawFeatures.cloudBackup
          : defaults.premiumFeatures.cloudBackup,
      advancedAnalytics:
        typeof rawFeatures.advancedAnalytics === "boolean"
          ? rawFeatures.advancedAnalytics
          : defaults.premiumFeatures.advancedAnalytics,
      advancedExports:
        typeof rawFeatures.advancedExports === "boolean"
          ? rawFeatures.advancedExports
          : defaults.premiumFeatures.advancedExports,
      whatsappReminders:
        typeof rawFeatures.whatsappReminders === "boolean"
          ? rawFeatures.whatsappReminders
          : defaults.premiumFeatures.whatsappReminders
    }
  };
}
...
function loadAdminConfig() {
  try {
    const storedValue = localStorage.getItem(adminConfigStorageKey);

    if (!storedValue) {
      return cloneDefaultAdminConfig();
    }

    return normalizeAdminConfig(JSON.parse(storedValue));
  } catch (error) {
    return cloneDefaultAdminConfig();
  }
}
...
function saveAdminConfig(config) {
  const normalizedConfig = normalizeAdminConfig(config);
  localStorage.setItem(adminConfigStorageKey, JSON.stringify(normalizedConfig));
  return normalizedConfig;
}

function formatAdminCurrency(amount, currencyCode) {
  try {
...
function getFeaturePresentation(config, featureKey) {
  const featureEnabled = Boolean(config.premiumFeatures[featureKey]);
  const copySet = featureDisplayCopy[featureKey];

  return {
    pill: featureEnabled ? "Premium" : "Included",
    toneClass: featureEnabled ? "warn is-premium" : "good is-included",
    description: featureEnabled
      ? copySet[config.launchMode === "live" ? "live" : "planned"]
      : copySet.included
  };
}

function getAdminConfigView(config) {
  const launchIsLive = config.launchMode === "live";
  const monthlyPrice = formatAdminCurrency(config.monthlyPrice, config.currency);
  const yearlyPrice = formatAdminCurrency(config.yearlyPrice, config.currency);
  const trialMessage =
    config.trialDays > 0
      ? `${config.trialDays}-day trial active before billing begins.`
      : "Billing begins immediately after sign-up.";

  return {
    heroState: launchIsLive ? "Live" : "Grow",
    modeLabel: launchIsLive ? "Paid plans live" : "Free growth mode",
    launchStatePill: launchIsLive ? "Live" : "Planned",
    launchStateClass: launchIsLive ? "good is-live" : "warn is-planned",
    monthlyPriceLabel: `${monthlyPrice} / month`,
    yearlyPriceLabel: `${yearlyPrice} / year`,
    monetizationMessage: launchIsLive
      ? "Premium plans are active. The app now uses the saved pricing, trial, and feature-lock rules everywhere this prototype exposes billing."
      : `TimeNest stays free while you grow adoption. Paid plans are prepared to start after ${config.freeAccessMonths} month${config.freeAccessMonths === 1 ? "" : "s"} whenever you switch them on.`,
    freeWindowLabel: launchIsLive
      ? trialMessage
      : `Charging is scheduled after ${config.freeAccessMonths} month${config.freeAccessMonths === 1 ? "" : "s"}, or any time you change launch mode to Charge Now.`,
    userPlanPill: launchIsLive ? "Free Trial" : "Free Member",
    userPlanDescription: launchIsLive
      ? `Users are on the launch offer with ${trialMessage.toLowerCase()}`
      : "Users are on the free growth plan while premium pricing is being prepared.",
    supportLabel: `Support: ${config.supportEmail}`,
...
function applyStatusPill(selector, label, className) {
  document.querySelectorAll(selector).forEach((element) => {
    element.textContent = label;
    element.classList.remove("good", "warn", "is-live", "is-planned", "is-included", "is-premium");
...
function applyAdminConfigToPage(config) {
  const view = getAdminConfigView(config);

  setTextContent("[data-admin-hero-state]", view.heroState);
  setTextContent("[data-monetization-mode-label]", view.modeLabel);
  setTextContent("[data-monthly-price-label]", view.monthlyPriceLabel);
  setTextContent("[data-yearly-price-label]", view.yearlyPriceLabel);
  setTextContent("[data-free-window-label]", view.freeWindowLabel);
  setTextContent("[data-monetization-message]", view.monetizationMessage);
  setTextContent("[data-user-plan-pill]", view.userPlanPill);
  setTextContent("[data-user-plan-description]", view.userPlanDescription);
  setTextContent("[data-admin-support-label]", view.supportLabel);
  setTextContent("[data-offer-headline]", view.offerHeadline);
  setTextContent("[data-offer-message]", view.offerMessage);
  applyStatusPill("[data-launch-state-pill]", view.launchStatePill, view.launchStateClass);

  document.querySelectorAll("[data-feature-copy]").forEach((element) => {
    const featureKey = element.dataset.featureCopy || "";

    if (!featureDisplayCopy[featureKey]) {
      return;
    }

    element.textContent = getFeaturePresentation(config, featureKey).description;
  });

  document.querySelectorAll("[data-feature-pill]").forEach((element) => {
    const featureKey = element.dataset.featurePill || "";

    if (!featureDisplayCopy[featureKey]) {
      return;
    }

    const presentation = getFeaturePresentation(config, featureKey);
    element.textContent = presentation.pill;
    element.classList.remove("good", "warn", "is-live", "is-planned", "is-included", "is-premium");
    presentation.toneClass.split(" ").forEach((token) => {
      if (token) {
        element.classList.add(token);
      }
    });
...
function populateAdminForm(form, config) {
  const checkedLaunchMode = form.querySelector(`input[name="launchMode"][value="${config.launchMode}"]`);

  if (checkedLaunchMode) {
    checkedLaunchMode.checked = true;
  }

  form.elements.freeAccessMonths.value = String(config.freeAccessMonths);
  form.elements.trialDays.value = String(config.trialDays);
  form.elements.monthlyPrice.value = String(config.monthlyPrice);
  form.elements.yearlyPrice.value = String(config.yearlyPrice);
  form.elements.currency.value = config.currency;
  form.elements.offerHeadline.value = config.offerHeadline;
  form.elements.offerMessage.value = config.offerMessage;
  form.elements.supportEmail.value = config.supportEmail;
  form.elements.featureCloudBackup.checked = config.premiumFeatures.cloudBackup;
  form.elements.featureAdvancedAnalytics.checked = config.premiumFeatures.advancedAnalytics;
  form.elements.featureAdvancedExports.checked = config.premiumFeatures.advancedExports;
  form.elements.featureWhatsappReminders.checked = config.premiumFeatures.whatsappReminders;
}

function readAdminForm(form) {
  return normalizeAdminConfig({
    launchMode: form.querySelector('input[name="launchMode"]:checked')?.value,
    freeAccessMonths: form.elements.freeAccessMonths.value,
    trialDays: form.elements.trialDays.value,
    monthlyPrice: form.elements.monthlyPrice.value,
    yearlyPrice: form.elements.yearlyPrice.value,
    currency: form.elements.currency.value,
    offerHeadline: form.elements.offerHeadline.value,
    offerMessage: form.elements.offerMessage.value,
    supportEmail: form.elements.supportEmail.value,
    premiumFeatures: {
      cloudBackup: form.elements.featureCloudBackup.checked,
      advancedAnalytics: form.elements.featureAdvancedAnalytics.checked,
      advancedExports: form.elements.featureAdvancedExports.checked,
      whatsappReminders: form.elements.featureWhatsappReminders.checked
    }
  });
}
...
const adminStatusMessage = document.querySelector("[data-admin-status]");

if (adminForm) {
  const initialAdminConfig = loadAdminConfig();
  populateAdminForm(adminForm, initialAdminConfig);
  applyAdminConfigToPage(initialAdminConfig);

  adminForm.addEventListener("submit", (event) => {
    event.preventDefault();

    const nextConfig = saveAdminConfig(readAdminForm(adminForm));
    applyAdminConfigToPage(nextConfig);

    if (adminStatusMessage) {
      adminStatusMessage.textContent =
        "Changes applied. Dashboard, settings, profile, and notifications now use this monetization setup in the current browser.";
      adminStatusMessage.classList.remove("is-muted");
      adminStatusMessage.classList.add("is-success");
    }
  });

  adminResetButton?.addEventListener("click", () => {
    const resetConfig = saveAdminConfig(cloneDefaultAdminConfig());
    populateAdminForm(adminForm, resetConfig);
    applyAdminConfigToPage(resetConfig);

    if (adminStatusMessage) {
      adminStatusMessage.textContent =
        "Prototype monetization settings reset to the default free-growth demo.";
      adminStatusMessage.classList.remove("is-success");
      adminStatusMessage.classList.add("is-muted");
    }
  });
} else {
  applyAdminConfigToPage(loadAdminConfig());
}
